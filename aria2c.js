        ...
        start() {
            ...
            aria2
                ...
                .catch(err => {
                    ...
                    if (!self.startup) {
                        self.startup = true;
                        require('child_process').execFile(opts.path + 'aria2c.exe', ['--conf-path=aria2.conf'], { cwd: opts.path }, function(error, stdout, stderr) {
                            console.log(error);
                            console.log(stdout)
                        })
        ...
        setItemUpdate(callback, ms = 1000){
             this.itemUpdate = setInterval(() => {
                ...
                for(let [guid, item] of Object.entries(downloading)){
                    a.push(['tellStatus', item[0], ['gid', 'totalLength', 'completedLength', 'downloadSpeed']]);
                    ids.push(item[1])
                }
        ...
        addUris(items, overwrite = false) {
            ...
            return new Promise(resolve => {
                ...
                for (let item of items) {
                    let { pathName, fileName, url } = item;
                    if (!overwrite && fs.existsSync(pathName + '\\' + fileName)) continue;
                    item = Object.assign({
